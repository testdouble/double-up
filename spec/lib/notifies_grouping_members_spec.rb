...
RSpec.describe NotifiesGroupingMembers do
  ...
  before(:example) do
    ...
    @builds_grouping_slack_content = double(Message::BuildsGroupingSlackContent)
    ...
    allow(Message::BuildsGroupingSlackContent).to receive(:new) { @builds_grouping_slack_content }
  end
  ...
  it "sends a single group slack message to all members" do
    ...
    expect(@builds_grouping_slack_content).to receive(:render)
