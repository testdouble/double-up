...
RSpec.describe "SlackSlashCommandController", type: :request do
  ...
  context "user asks to be unavailable" do
    let(:text) { "unavailable" }

    scenario "responds to `/doubleup unavailable`" do
  ...
  context "user asks to be available" do
    let(:text) { "available" }

    scenario "responds to available when user is already available" do
      expect(@updates_participant_availability).to receive(:call)
    ...
    scenario "responds to available when user is unavailable" do
      expect(@updates_participant_availability).to receive(:call)
  ...
  context "any other text command is given" do
    let(:text) { "ping" }

    scenario "responds to anything with pong" do
